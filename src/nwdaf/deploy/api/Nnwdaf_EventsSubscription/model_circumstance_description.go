/*
 * Nnwdaf_EventsSubscription
 *
 * Nnwdaf_EventsSubscription Service API. © 2020, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 
 *
 * API version: 1.2.0-alpha.2
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// CircumstanceDescription struct for CircumstanceDescription
type CircumstanceDescription struct {
	Freq *map[string]interface{} `json:"freq,omitempty"`
	Tm *map[string]interface{} `json:"tm,omitempty"`
	LocArea *map[string]interface{} `json:"locArea,omitempty"`
	Vol *map[string]interface{} `json:"vol,omitempty"`
}

// NewCircumstanceDescription instantiates a new CircumstanceDescription object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCircumstanceDescription() *CircumstanceDescription {
	this := CircumstanceDescription{}
	return &this
}

// NewCircumstanceDescriptionWithDefaults instantiates a new CircumstanceDescription object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCircumstanceDescriptionWithDefaults() *CircumstanceDescription {
	this := CircumstanceDescription{}
	return &this
}

// GetFreq returns the Freq field value if set, zero value otherwise.
func (o *CircumstanceDescription) GetFreq() map[string]interface{} {
	if o == nil || o.Freq == nil {
		var ret map[string]interface{}
		return ret
	}
	return *o.Freq
}

// GetFreqOk returns a tuple with the Freq field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CircumstanceDescription) GetFreqOk() (*map[string]interface{}, bool) {
	if o == nil || o.Freq == nil {
		return nil, false
	}
	return o.Freq, true
}

// HasFreq returns a boolean if a field has been set.
func (o *CircumstanceDescription) HasFreq() bool {
	if o != nil && o.Freq != nil {
		return true
	}

	return false
}

// SetFreq gets a reference to the given map[string]interface{} and assigns it to the Freq field.
func (o *CircumstanceDescription) SetFreq(v map[string]interface{}) {
	o.Freq = &v
}

// GetTm returns the Tm field value if set, zero value otherwise.
func (o *CircumstanceDescription) GetTm() map[string]interface{} {
	if o == nil || o.Tm == nil {
		var ret map[string]interface{}
		return ret
	}
	return *o.Tm
}

// GetTmOk returns a tuple with the Tm field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CircumstanceDescription) GetTmOk() (*map[string]interface{}, bool) {
	if o == nil || o.Tm == nil {
		return nil, false
	}
	return o.Tm, true
}

// HasTm returns a boolean if a field has been set.
func (o *CircumstanceDescription) HasTm() bool {
	if o != nil && o.Tm != nil {
		return true
	}

	return false
}

// SetTm gets a reference to the given map[string]interface{} and assigns it to the Tm field.
func (o *CircumstanceDescription) SetTm(v map[string]interface{}) {
	o.Tm = &v
}

// GetLocArea returns the LocArea field value if set, zero value otherwise.
func (o *CircumstanceDescription) GetLocArea() map[string]interface{} {
	if o == nil || o.LocArea == nil {
		var ret map[string]interface{}
		return ret
	}
	return *o.LocArea
}

// GetLocAreaOk returns a tuple with the LocArea field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CircumstanceDescription) GetLocAreaOk() (*map[string]interface{}, bool) {
	if o == nil || o.LocArea == nil {
		return nil, false
	}
	return o.LocArea, true
}

// HasLocArea returns a boolean if a field has been set.
func (o *CircumstanceDescription) HasLocArea() bool {
	if o != nil && o.LocArea != nil {
		return true
	}

	return false
}

// SetLocArea gets a reference to the given map[string]interface{} and assigns it to the LocArea field.
func (o *CircumstanceDescription) SetLocArea(v map[string]interface{}) {
	o.LocArea = &v
}

// GetVol returns the Vol field value if set, zero value otherwise.
func (o *CircumstanceDescription) GetVol() map[string]interface{} {
	if o == nil || o.Vol == nil {
		var ret map[string]interface{}
		return ret
	}
	return *o.Vol
}

// GetVolOk returns a tuple with the Vol field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CircumstanceDescription) GetVolOk() (*map[string]interface{}, bool) {
	if o == nil || o.Vol == nil {
		return nil, false
	}
	return o.Vol, true
}

// HasVol returns a boolean if a field has been set.
func (o *CircumstanceDescription) HasVol() bool {
	if o != nil && o.Vol != nil {
		return true
	}

	return false
}

// SetVol gets a reference to the given map[string]interface{} and assigns it to the Vol field.
func (o *CircumstanceDescription) SetVol(v map[string]interface{}) {
	o.Vol = &v
}

func (o CircumstanceDescription) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Freq != nil {
		toSerialize["freq"] = o.Freq
	}
	if o.Tm != nil {
		toSerialize["tm"] = o.Tm
	}
	if o.LocArea != nil {
		toSerialize["locArea"] = o.LocArea
	}
	if o.Vol != nil {
		toSerialize["vol"] = o.Vol
	}
	return json.Marshal(toSerialize)
}

type NullableCircumstanceDescription struct {
	value *CircumstanceDescription
	isSet bool
}

func (v NullableCircumstanceDescription) Get() *CircumstanceDescription {
	return v.value
}

func (v *NullableCircumstanceDescription) Set(val *CircumstanceDescription) {
	v.value = val
	v.isSet = true
}

func (v NullableCircumstanceDescription) IsSet() bool {
	return v.isSet
}

func (v *NullableCircumstanceDescription) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCircumstanceDescription(val *CircumstanceDescription) *NullableCircumstanceDescription {
	return &NullableCircumstanceDescription{value: val, isSet: true}
}

func (v NullableCircumstanceDescription) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCircumstanceDescription) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


